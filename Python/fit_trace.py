...
def fit_trace(time, trace, lowres_time, lowres_trace): # Given a trace, this function calculates the number of sigmoids and their parameters to fit the trace.
	...
	switching_points = [];
	min_height = 0.02;
	for i in range(0, len(trace)): # Iterate through the derivative array and identify small portions (size of the filter) where the gradient is constant and not zero.
...
path = sys.argv[1];
data = aux.read_file(path, sys.maxint); # Read the given file in its full length.
